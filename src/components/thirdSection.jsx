...
const TestimonialCarousel = () => {
  ...
  const handleMouseLeave = () => {
    if (!selectedTestimonial) setIsAutoScrollPaused(false);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 py-16 text-center">
      ...
      <div
        className="relative hidden sm:flex items-center justify-center mb-10 h-[420px]"
      ...
      >
        <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
          {testimonials.map((testimonial, index) => {
            const baseClasses =
              "absolute bg-white rounded-2xl p-6 shadow-lg transition-all duration-500 ease-in-out opacity-0 transform scale-75 w-[85%] max-w-sm sm:w-72 sm:h-[300px] flex flex-col justify-between";
            const activeClasses =
              "opacity-100 scale-110 z-30 sm:w-[350px] sm:h-[340px] shadow-2xl";
            const prevClasses =
              "opacity-70 -translate-x-40 sm:-translate-x-52 scale-90 z-20";
            const nextClasses =
              "opacity-70 translate-x-40 sm:translate-x-52 scale-90 z-20";
            const hiddenClasses = "opacity-0 scale-50 z-10";

            let appliedClasses = baseClasses;
            if (index === currentIndex) appliedClasses += " " + activeClasses;
            else if (
              index ===
            ...
            else appliedClasses += " " + hiddenClasses;

            return (
              <div key={testimonial.id} className={appliedClasses}>
                <div className="flex-grow overflow-hidden">
                  <div className="text-gray-700 text-base sm:text-lg italic mb-6 leading-relaxed">
      ...
      <div className="flex flex-col gap-6 sm:hidden">
        {testimonials.map((testimonial) => (
          <div
            key={testimonial.id}
            className="bg-white rounded-2xl p-6 shadow-lg w-full flex flex-col gap-4"
          >
